...
详见 `technical_report_enriched.docx`（增强版技术报告副本），该报告在原始技术报告基础上新增了以下内容：

- **数据集扩充前后的详细对比**：量化了训练集（1,748→7,362）、验证集（250→1,052）、测试集（499→2,104）的具体变化
- **测试集公平性分析（第 2.2.2 节）**：指出 v1.06/v1.07 的性能提升（90.00%→96.53%/97.48%）综合了数据集扩充、训练优化与**评估集变更**三个因素，具体包括：
  - **基准不一致**：v1.03 的 90.00% 与 v1.06 的 96.53% 分别来自不同测试集（499 张 vs 2,104 张），并非严格公平的横向对比
  - **数据泄露风险**：扩充数据与测试集同源（均来自 omasteam/realwaste），可能产生隐式过拟合优势
  - **统计可靠性**：测试集从 499 张增至 2,104 张，评估结果的置信区间更窄，但同时也改变了评价基准
- **消融实验估算**：量化了各项优化技术的预估贡献度

> 该补充分析旨在更严谨地反映实验设计的局限性，为后续实验改进提供方向参考。
